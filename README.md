...
- `pipeline-configuration-file-path` is an absolute path to a pipeline configuration json file. This contains a pipeline name that infers which pipeline to run (Dadaab or Kakuma) 
...
### 7. Upload logs
This stage uploads the archive produced in the last step and memory/cpu profile logs of the generate outputs stage
