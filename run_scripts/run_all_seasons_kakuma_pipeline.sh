...
if [[ $# -ne 9 ]]; then
    echo "Usage: ./run_all_seasons_pipeline.sh"
...
./1_all_seasons_kakuma_coda_get.sh "$CODA_PULL_CREDENTIALS_PATH" "$CODA_TOOLS_ROOT" "$DATA_ROOT"
...
./4_all_seasons_kakuma_coda_add.sh "$CODA_PUSH_CREDENTIALS_PATH" "$CODA_TOOLS_ROOT" "$DATA_ROOT"
